...
async def challengeList(message):
    await message.send('Obteniendo nuestra lista de Retos')
...
@bot.command(name='play', help='connects bot and plays music')
async def play(message):
    url = message.message.content.split(' ')[1]
    await message.channel.send(play_music(url))
